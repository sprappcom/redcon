...
func (c *Conn) WriteArray(count int) {
	c.buf.Write(AppendArray(nil, count))
}

func (c *Conn) WriteBulkString(bulk string) {
	c.buf.Write(AppendBulkString(nil, bulk))
}

func (c *Conn) WriteString(str string) {
	c.buf.Write(AppendString(nil, str))
}

func (c *Conn) WriteInt(num int) {
	c.buf.Write(AppendInt(nil, int64(num)))
}

func (c *Conn) WriteNull() {
	c.buf.Write(AppendNull(nil))
}
...
func (c *Conn) ReadCommand() (Command, error) {
	...
	c.buf.Write(data)
	cmds, _, err := ReadCommands(c.buf.Bytes())
	if err != nil {
		return Command{}, err
	}
	if len(cmds) > 0 {
...
func (c *Conn) WriteError(msg string) {
	c.buf.Write(AppendError(nil, msg))
}
...
type redHub struct {
	...
	handler      func(cmd Command, out []byte) ([]byte, Action)
...
func (rs *redHub) OnTraffic(c gnet.Conn) gnet.Action {
	...
	if !ok {
		out := AppendError(nil, "ERR Client is closed")
		c.Write(out)
		return gnet.Close
	}
	frame, _ := c.Next(-1)
	cb.buf.Write(frame)
	cmds, lastbyte, err := ReadCommands(cb.buf.Bytes())
	if err != nil {
		out := AppendError(nil, "ERR "+err.Error())
		c.Write(out)
		return gnet.None
	}
